...
printModules()
{
    ...
    while (ptr)
    {
        printf("%-15s | %-11s | %s\n", ptr->module, ptr->type, ptr->info->version);
        ptr = ptr->next;
    }
}
...
dprintAllModules(DCB *dcb)
{
    ...
    while (ptr)
    {
        dcb_printf(dcb, "%-15s | %-15s | %-7s ", ptr->module, ptr->type, ptr->info->version);
        if (ptr->info)
    ...
    *buffer = gwbuf_alloc(report_max_bytes);
    ...
    while (ptr)
    {
        snprintf(data_ptr, _NOTIFICATION_REPORT_ROW_LEN * 2,
                 "module_%s_type\t%s\nmodule_%s_version\t%s\n",
                 ptr->module, ptr->type, ptr->module, ptr->info->version);
        data_ptr += strlen(data_ptr);
